...
const typeMap: { [key: string]: { label: string; color: string } } = {
  ...
  absentee: { label: 'Absentee', color: 'bg-[#F59E0B]/10 text-[#F59E0B] border-[#F59E0B]/20' },
  productivity: { label: 'Productivity', color: 'bg-[#10B981]/10 text-[#10B981] border-[#10B981]/20' },
}
  ...
  return (
    <div className="glass overflow-hidden rounded-3xl border border-[#334155]/50 group hover:border-[#2563EB]/30 transition-all duration-300">
      <div className="relative aspect-video bg-[#0F172A] overflow-hidden">
        {(alert.screenshot_url || alert.screenshotUrl) ? (
          <Image 
            src={(alert.screenshot_url || alert.screenshotUrl)!} 
        ...
        <div className="absolute top-3 right-3">
          <span className={clsx('px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border backdrop-blur-md', typeMap[alert.type || alert.alertType]?.color)}>
            {typeMap[alert.type || alert.alertType]?.label}
          </span>
        </div>
      </div>
      ...
      <div className="p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h4 className="text-[#F1F5F9] font-semibold text-sm">Camera {alert.camera_id || alert.cameraId}</h4>
        ...
        <div className="flex items-center gap-2 pt-2 border-t border-[#334155]/30">
          ...
          <button 
            onClick={() => onIgnore?.(alert._id || alert.id || '')}
