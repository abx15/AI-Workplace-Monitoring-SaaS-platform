...
export default function AdminDashboard() {
  ...
  const stats = [
    { title: 'Total Employees', value: '1,280', icon: Users, color: 'blue', trend: { value: 12, isUp: true } },
    { title: 'Active Cameras', value: cameras.filter(c => c.status === 'active').length, icon: Video, color: 'green' },
    { title: 'Alerts Today', value: '24', icon: Bell, color: 'red', trend: { value: 5, isUp: false } },
    { title: 'Active Workers', value: '842', icon: Activity, color: 'yellow', trend: { value: 8, isUp: true } },
  ]

  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-1">
        <h1 className="text-3xl font-bold text-[#F1F5F9] tracking-tight">Overview</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass rounded-3xl border border-[#334155]/50 p-6">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-lg font-bold text-[#F1F5F9]">Alert Trends (Last 7 Days)</h3>
        ...
        <div className="glass rounded-3xl border border-[#334155]/50 p-6">
          <h3 className="text-lg font-bold text-[#F1F5F9] mb-8">Worker Activity</h3>
          <div className="h-[300px] w-full">
            <ActivityChart />
          </div>
        </div>
          ...
          <div className="glass rounded-3xl border border-[#334155]/50 overflow-hidden">
            <div className="divide-y divide-[#334155]/50">
              {cameras.slice(0, 6).map((camera) => (
                <div key={camera.id} className="p-4 flex items-center justify-between hover:bg-[#334155]/20 transition-all cursor-pointer group">
